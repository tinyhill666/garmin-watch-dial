...
def gen_text_font(ttf, size, charset, name):
    ...
    # 每字形: [ch, 渲染用x0, y0, w, h, 记录用xoffset, xadvance]
    glyphs = []
    for ch in charset:
        x0, y0, x1, y1 = font.getbbox(ch)
        w, h = x1 - x0, y1 - y0
        adv = round(font.getlength(ch))
        glyphs.append([ch, x0, y0, w, h, x0, adv])

    # 数字强制等宽（tabular）：统一步进为数字最大步进，仅调整记录里的 xoffset
    # 让字形在步进内居中；图集内的绘制位置仍用原始 bbox，避免字形溢出串位
    digit_adv = max(g[6] for g in glyphs if g[0].isdigit())
    for g in glyphs:
        if g[0].isdigit():
            g[5] += (digit_adv - g[6]) // 2
            g[6] = digit_adv
    ...
    chars = []
    cx = PAD
    for ch, x0, y0, w, h, xoff, adv in glyphs:
        if w > 0 and h > 0:
            draw.text((cx - x0, PAD - y0), ch, font=font, fill=(255, 255, 255, 255))
        chars.append(
...
def quantize_alpha(atlas):
    """锐化并量化 alpha 到 4 级（0/85/170/255）。

    MIP 屏只有 RGB222 64 色，白字黑底的中间色阶仅 0x55/0xAA 两级灰；
    宽羽化边会渲染成脏边。先用曲线压掉弱羽化、再量化，边缘更干净。
    """
    r, g, b, a = atlas.split()
    lut = []
    for v in range(256):
        s = (v - 48) * 255 / (208 - 48)  # 收窄过渡带
        s = max(0, min(255, s))
        lut.append(int(round(s / 85.0)) * 85)
    a = a.point(lut)
...
if __name__ == "__main__":
    ...
    gen_text_font("TitilliumWeb-SemiBold.ttf", 26,
                  " -/°0123456789ABCDEFGHIJLMNOPRSTUVWY", "tempo-semibold-26")
    gen_text_font("TitilliumWeb-SemiBold.ttf", 20, "0123456789", "tempo-semibold-20")
