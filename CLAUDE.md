...
- 布局：顶行心率组+电池图标 / 双色大字时间（时白、分绿 #55FF00，无冒号，Chakra Petch Bold 76px）/
  秒（专用 28px）贴分右缘底对齐 / 日期行（DateFont 30px，日数绿）/ 底部三格 天气|步数|压力（SmallFont 22px）
  / 下沿步数目标进度弧（r=118 pen6，230°–310°，灰轨道+主题色进度）
- 字体角色：time(76 大字) / date(30 日期，放大减 W 锯齿) / data(26 心率) / small(22 底部三格) / sec(28) / icons(Material Symbols)
  —— 日期与底部各用独立字号：日期行横向空间足可放大，底部三格窄需缩小避免相互及与下半圆边挤碰
- 字体：Chakra Petch（几何切角，直线为主，SIL OFL）—— 比圆体（Barlow/Titillium）在 MIP 屏锯齿少很多；
  Chakra 偏宽，时间字号取 76（不是 92）才能保证「时+分+秒」整体不越圆界（曾经的教训见下）；
  时间块在 onLayout 里用秒右侧的圆界余量整体右移，避免时数字贴左缘
